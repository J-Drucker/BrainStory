...
import 'ant_cnt_import_stub.dart'
    if (dart.library.io) 'ant_cnt_import_io.dart' as impl;
...
AntCntImportData parseAntCntPayload(
  ...
  if (channelCount <= 0 || sampleCount <= 0) {
    throw const FormatException('ANT CNT payload did not include a valid data shape.');
  }
  ...
  for (int channelIndex = 0; channelIndex < channelCount; channelIndex++) {
    for (int sampleIndex = 0; sampleIndex < sampleCount; sampleIndex++) {
      channelSamples[channelIndex][sampleIndex] =
  ...
  return AntCntImportData(
    ...
    markers: (payload['markers'] as List<dynamic>? ?? const <dynamic>[])
...
Map<String, dynamic> decodeAntCntPayload(String stdout) {
  ...
  if (decoded is! Map<String, dynamic>) {
    throw const FormatException('ANT CNT importer returned malformed metadata.');
  }
  return decoded;
}
